...
function Experience() {
  ...
  return (
    <section className="relative max-w-screen min-h-screen  px-20 pt-25 pb-10 experience">
      <h1 className="text-5xl font-bold mb-15 ">My Work Experience</h1>
      <div role="list" className="grid grid-cols-3  gap-7 cursor-pointer">
        {listOfExperience.map((el, i) => (
          ...
          >
            ...
            />
            <div
              className={`absolute  left-1/2 -translate-x-1/2 w-[90%]   flex flex-col gap-0 bg-white text-black bg-opacity-50  p-4 rounded-xl  transition-all duration-500 ${
                hoveredItem === i
                  ? "bottom-10 opacity-100"
                  : "opacity-0 bottom-5"
              } `}
            >
              <p className="text-muted mb-2">{el.category}</p>
              <p className="text-3xl font-semibold capitalize">{el.title}</p>
